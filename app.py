# ---------- app.py (KHT AI Auto-Grader with Teacher-Specific Dashboard + Analytics) ----------
    ...
    if login_type=="Teacher":
        ...
        if login_btn:
            if teacher_user in teachers and teachers[teacher_user]==hash_password(teacher_pass):
                ...
                st.session_state.username=teacher_user
...
# ---------- Teacher/Admin Common: Upload & Grade ----------
st.subheader("📥 Upload Answer Key")
key_file = st.file_uploader("Upload Teacher Answer Key", type=["txt","jpg","jpeg","png"])
if key_file:
...
batch_mode = st.checkbox("Enable Batch Grading")
...
if not batch_mode:
    ...
    exam_file = st.file_uploader("Upload Student Exam", type=["jpg","png","jpeg"])
    if exam_file:
        ...
        if st.button("Grade Answer"):
            if not all([student_name,student_id,department,subject]):
                ...
                result={"Student ID":student_id,"Name":student_name,"Teacher":st.session_state.username,"Department":department,
                ...
                st.markdown("<p class='notification'>Result saved!</p>", unsafe_allow_html=True)
else:
    batch_files = st.file_uploader("Upload Multiple Exams", type=["jpg","png","jpeg"], accept_multiple_files=True)
    if batch_files and st.button("Grade All Exams"):
        ...
        for file in batch_files:
            ...
            score, feedback = grade_with_answer_key(model_answer, student_answer)
            results.append({"Student ID":student_id,"Name":student_name,"Teacher":st.session_state.username,"Department":department,
                            "Subject":subject,"Answer":student_answer,"Score":score,
                            "Feedback":feedback,"Timestamp":datetime.now().strftime("%Y-%m-%d %H:%M:%S")})
...
if results_files:
    ...
    if st.session_state.role=="Teacher":
        combined_df = combined_df[combined_df["Teacher"]==st.session_state.username]
    
    st.markdown("### Score Distribution")
    fig = px.histogram(combined_df, x="Score", nbins=20, title="Student Score Distribution")
    st.plotly_chart(fig, use_container_width=True)
    
    st.markdown("### Average Score by Department")
    avg_df = combined_df.groupby("Department")["Score"].mean().reset_index()
    st.dataframe(avg_df)
else:
    st.markdown("<p class='notification'>No results available yet for analytics.</p>", unsafe_allow_html=True)
