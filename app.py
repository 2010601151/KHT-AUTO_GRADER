...
def load_pending_teachers():
    if os.path.exists("pending_teachers.json"):
        with open("pending_teachers.json", "r") as f:
            return json.load(f)
    return {}

def save_pending_teachers(data):
    with open("pending_teachers.json", "w") as f:
        json.dump(data, f)
...
if login_type == "Teacher":
    ...
    if register_btn:
        if teacher_user in teachers or teacher_user in pending_teachers:
            st.sidebar.warning("❌ Username already exists or pending approval.")
            ...
            st.sidebar.success("✅ Teacher registered. Waiting for admin approval.")
        else:
...
# ---------- Sidebar Navigation ----------
page_options = ["📥 Upload Answer Key",
    ...
    if exam_file:
        ...
        if st.button("Grade Answer"):
            if not model_answer:
                ...
                st.info("Detailed Feedback below:")
...
if page == "📈 Analytics":
    ...
    if os.path.exists(analytics_path):
        ...
        st.info("ℹ️ No results available for this department/subject yet. Upload and grade exams first.")
...
if page == "👥 Manage Teachers" and st.session_state.role == "Admin":
    ...
    for username in list(pending_teachers.keys()):
        col1, col2 = st.columns([3,1])
        col1.write(username)
        if col2.button(f"Approve {username}"):
            teachers[username] = pending_teachers.pop(username)
            save_teachers(teachers)
            save_pending_teachers(pending_teachers)
            st.experimental_rerun()
        if col2.button(f"Remove {username}"):
