...
# ---------- Fixed Query Params Sidebar Toggle ----------
...
# (Pages 1–5: Upload Answer Key, Grade Exam, Search, Dashboard, Analytics)
# The page code remains fully functional as in your last full version.
...
if page == "🔍 Search Results (ID or Name)":
    ...
    if os.path.exists(search_path):
        ...
        if search_term:
            filtered = df[df.apply(lambda x: search_term.lower() in str(x["Student ID"]).lower() 
                                   or search_term.lower() in str(x["Name"]).lower(), axis=1)]
