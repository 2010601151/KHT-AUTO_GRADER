...
if st.session_state.role=="Admin":
    st.sidebar.markdown("### 👩‍🏫 Manage Teacher Accounts")
    teachers = load_teachers()
    if teachers:
        st.sidebar.markdown("#### Registered Teachers")
        for t_user in teachers:
            st.sidebar.markdown(f"- {t_user} | {teachers[t_user][:10]}... (hashed password)")
    # Add Teacher
    new_teacher_user = st.sidebar.text_input("New Teacher Username", key="admin_new_user")
    new_teacher_pass = st.sidebar.text_input("New Teacher Password", type="password", key="admin_new_pass")
    add_clicked = st.sidebar.button("Add Teacher")
    if add_clicked:
        if new_teacher_user in teachers:
            ...
            save_teachers(teachers)
            st.sidebar.markdown(f"<p class='notification'>✅ Teacher '{new_teacher_user}' added successfully!</p>", unsafe_allow_html=True)
        else:
            st.sidebar.markdown("<p class='notification'>⚠️ Enter both username and password.</p>", unsafe_allow_html=True)
    # Delete Teacher
    if teachers:
        teacher_to_delete = st.sidebar.selectbox("Select Teacher to Delete", options=list(teachers.keys()), key="delete_teacher_select")
        delete_clicked = st.sidebar.button("Delete Selected Teacher")
        if delete_clicked:
            if teacher_to_delete in teachers:
                del teachers[teacher_to_delete]
                save_teachers(teachers)
                st.sidebar.markdown(f"<p class='notification'>✅ Teacher '{teacher_to_delete}' deleted successfully!</p>", unsafe_allow_html=True)
            else:
                st.sidebar.markdown("<p class='notification'>❌ Teacher not found.</p>", unsafe_allow_html=True)
...
if page=="🔍 Search Results (ID or Name)":
    ...
    if os.path.exists(search_path):
        ...
        if search_term:
            ...
            else: st.markdown("<p class='notification'>No results found for this search.</p>", unsafe_allow_html=True)
